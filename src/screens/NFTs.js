...
var collectionIndices = [1,2,3,4,5,6]
var NFTs = []

function fetchNFTs(collectionIndices, state){
   
  var currentIndex = collectionIndices.pop();

  // alert(currentIndex)

 var newCollectionIndices = collectionIndices;

//  alert(newCollectionIndices.length)

  fetch('https://radixnft.art/data/item/3/'+currentIndex, {
      method: 'GET',
      headers: {
      ...
      NFTs.push({title:json.Items[0].Metadata.description, subtitle: "by: "+json.Items[0].Metadata.artist, illustration:"https://gateway.pinata.cloud/ipfs/"+json.Items[0].Metadata.real_element})
      
      // alert(JSON.stringify(NFTs))
      
      if(collectionIndices.length != 0){
        fetchNFTs(newCollectionIndices, state)
      } else{
        state.setState({galleryNFTs: NFTs})
      }
    }).catch((error) => {
        console.error(error);
    });
...
export default class example extends Component {
    ...
    constructor (props) {
        ...
        fetchNFTs(collectionIndices, this)
        // alert(JSON.stringify(NFTs))
    }
    ...
    mainExample (number, title) {
        ...
        const { galleryNFTs } = this.state;

        return (
            <View style={styles.exampleContainer}>
                {/* <Text style={styles.title}>{`Example ${number}`}</Text> */}
                <Text style={styles.title}>{"Skullzos Collection"}</Text>
